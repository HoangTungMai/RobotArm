...
sound_path = ['./sounds/Start_sound.mp3', 
              './sounds/End_sound.mp3']
...
def compute_dir(delta_x, delta_y, delta_z):
    ...
    z_dir = "Same" if delta_z == 0 else ("In" if delta_z > 0 else "Out" )
    ...
    d_x_norm = int(delta_x*t_x)
    d_y_norm = int(delta_y*t_y)
    
    d_z_norm = int(delta_z*t_z)
...
def layout(image, gripper_mode = None, dir = None):
    ...
    if active_mode == True:
        borded_img = cv2.copyMakeBorder(image, top, bottom, left, right, cv2.BORDER_CONSTANT, value = (255, 0, 0))
    else:
...
with mp_hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:    
            x_0, y_0, z_0 = get_landmark(results, HandLandmark.WRIST)
            x_4, y_4, z_4 = get_landmark(results, HandLandmark.THUMB_TIP) 
            x_8, y_8, z_8 = get_landmark(results, HandLandmark.INDEX_FINGER_TIP) 
            _, y_12, _ = get_landmark(results, HandLandmark.MIDDLE_FINGER_TIP) 
            _, y_16, _ = get_landmark(results, HandLandmark.RING_FINGER_TIP) 
            _, y_20, _ = get_landmark(results, HandLandmark.PINKY_TIP) 
            _, offset, _ = get_landmark(results, HandLandmark.MIDDLE_FINGER_PIP) 
            x_13,y_13,z_0 = get_landmark(results, HandLandmark.PINKY_MCP) 
        ...
        if gripper_mode:
                    ...
                    roi = frame[-size-450:-450, -size-600:-600] 
                    roi[np.where(mask)] = 0
                    roi += logo 
        ...
        if dir[0][0] == "Right":
            logo_right = cv2.imread('.\images\d_right.png') 
            size = 50
            logo_right = cv2.resize(logo_right, (size, size)) 

            # Create a mask of logo 
            img2gray = cv2.cvtColor(logo_right, cv2.COLOR_BGR2GRAY) 
            ret, mask = cv2.threshold(img2gray, 1, 255, cv2.THRESH_BINARY) 
            roi = frame[-size-250:-250, -size-10:-10] 
            roi[np.where(mask)] = 0
            roi += logo_right 
        
        if dir[0][0] == "Left":
            logo_right = cv2.imread('.\images\d_left.png') 
            size = 50
            logo_right = cv2.resize(logo_right, (size, size)) 
            ...
            img2gray = cv2.cvtColor(logo_right, cv2.COLOR_BGR2GRAY) 
            ret, mask = cv2.threshold(img2gray, 1, 255, cv2.THRESH_BINARY) 
            roi = frame[-size-250:-250, -size-600:-600] 
            roi[np.where(mask)] = 0
            roi += logo_right 
          
        if dir[1][0] == "Up":
            logo_right = cv2.imread('.\images\d_up.png') 
            size = 50
            logo_right = cv2.resize(logo_right, (size, size)) 
            ...
            img2gray = cv2.cvtColor(logo_right, cv2.COLOR_BGR2GRAY) 
            ret, mask = cv2.threshold(img2gray, 1, 255, cv2.THRESH_BINARY) 
            roi = frame[-size-450:-450, -size-300:-300] 
            roi[np.where(mask)] = 0
            roi += logo_right 
        
        if dir[1][0] == "Down":
            logo_right = cv2.imread('.\images\d_down.png') 
            size = 50
            logo_right = cv2.resize(logo_right, (size, size)) 
            ...
            img2gray = cv2.cvtColor(logo_right, cv2.COLOR_BGR2GRAY) 
            ret, mask = cv2.threshold(img2gray, 1, 255, cv2.THRESH_BINARY) 
            roi = frame[-size-10:-10, -size-300:-300] 
            roi[np.where(mask)] = 0
            roi += logo_right 
            
        if dir[2][0] == "In":
            logo_right = cv2.imread('.\images\d_in.png') 
            size = 50
            logo_right = cv2.resize(logo_right, (size, size)) 
            ...
            img2gray = cv2.cvtColor(logo_right, cv2.COLOR_BGR2GRAY) 
            ret, mask = cv2.threshold(img2gray, 1, 255, cv2.THRESH_BINARY) 
            roi = frame[-size-450:-450, -size-10:-10] 
            roi[np.where(mask)] = 0
            roi += logo_right 
        
        if dir[2][0] == "Out":
            logo_right = cv2.imread('.\images\d_out.png') 
            size = 50
            logo_right = cv2.resize(logo_right, (size, size)) 
