...
def get_landmark(detector, idx):
    hand_landmark = detector.hand_world_landmarks[0]
    x = hand_landmark[idx].x * 480 / 100
    y = hand_landmark[idx].y * 640 / 100
    z = hand_landmark[idx].z * 10
    return -x, -y, z
...
def compute_dir(delta_x, delta_y, delta_z):
    ...
    d_y_norm = int(delta_y*t*2)
    d_z_norm = int(delta_z*t * 0.4)
...
while cap.isOpened():
    ...
    # If hand landmarks are detection_resultdetected, iterate through them
    if detection_result.hand_world_landmarks:  
    
        x_4, y_4, z_4 = get_landmark(detection_result, 4) # Thumb tip
        x_8, y_8, z_8 = get_landmark(detection_result, 8) # Index finger tip
        _, y_12, _ = get_landmark(detection_result, 12) # Middle finger tip
        _, y_16, _ = get_landmark(detection_result, 16) # Ring finger tip
        _, y_20, _ = get_landmark(detection_result, 20) # Ring finger tip
        _, offset, _ = get_landmark(detection_result, 10) # Ring finger tip 
        x_0, y_0, z_0 = get_landmark(detection_result, 0) # Wrist 
        
        print(f"x_0 = {x_0}, y_0 = {y_0}, z_0 = {z_0}")
        str_z0 = "Z0 = " + str(z_0)
        ...
        print("active_mode = ", active_mode)
        print("\n")
        print(f"d_x = {delta_x_filterd}, d_y = {delta_y_filterd}, d_z = {delta_z_filterd}")
